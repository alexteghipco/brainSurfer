...
%
% Last Modified by GUIDE v2.5 17-Sep-2019 16:44:11
...
if hObject.Value == 5 
    ...
    %brainFile = uipickfiles('FilterSpec',{'*.inflated'; '*.nii'; '*.gii'},'Prompt','Select one or two surface(s) to plot.');
    brainFile = uipickfiles('Type', {'*.inflated','freesurfer format';'*.gii','gifti surface'},'Prompt','Select one or two surface(s) to plot.');
    uiwait(warndlg('Please select your curvature files in the same order as the surfaces!'));
    %curvFile = uipickfiles('FilterSpec','*.curv','Prompt','Select curvatures for each of your surface(s)');
    %curvFile = uipickfiles('Type',{'*.nii','cifti format';'*.curv','freesurfer curvature files';'*.gii','gifti surface'},'Prompt','Select curvatures for each of your surface(s).');
    curvFile = uipickfiles('REFilter','\.nii$|\.nii.gz$|\.gii$','Prompt','Select curvatures for each of your surface(s).');
    ...
    if length(brainFile) > 1 % if you selected two files lets load both in
        [path1,name1,ext1] = fileparts(brainFile{1});
        [path2,name2,ext2] = fileparts(brainFile{2});
        [path3,name3,ext3] = fileparts(curvFile{1});
        if length(curvFile) > 1
            [path4,name4,ext4] = fileparts(curvFile{2});
        end
        ...
        switch ext1
            ...
            case '.gii'
                try
                    hdr = gifti(brainFile{1});
                    vert1 = hdr.vertices;
                    face1 = hdr.faces;
                catch
                    disp('There are no vertices and/or faces in your gii file (mesh)')
                end
        end
        ...
        if length(curvFile) == 1
            switch ext3
                case '.nii'
                    try
                        cH = 1;
                        hdr = cifti_read(curvFile{1});
                        curv1Tmp = zeros([size(vert1,1),1]);
                        curv1Tmp(hdr.diminfo{1}.models{1}.vertlist + 1) = hdr.cdata(hdr.diminfo{1}.models{1}.start:hdr.diminfo{1}.models{1}.start+hdr.diminfo{1}.models{1}.count-1);
                        curv2Tmp = zeros([size(vert2,1),1]);
                        curv2Tmp(hdr.diminfo{1}.models{2}.vertlist + 1) = hdr.cdata(hdr.diminfo{1}.models{2}.start:hdr.diminfo{1}.models{2}.start+hdr.diminfo{1}.models{2}.count-1);

                        handles.w = warndlg('Assuming CIFTI file is from HCP, in which case we need to invert the sulcal/gyral info. If this is not the case, just manually change sulcal/gyral colors in surface menu');
                        %curv1Tmp = -1*curv1Tmp;
                        %curv2Tmp = -1*curv2Tmp;
                        
                        % if this is the case, you need to figure out whether to
                        % flip curv1 or curv2 -- this will depend on cifti
                        % file metadata AND on whether left or right
                        % surface was loaded first. To make this code sane
                        % later, just define hemispheres before you do this
                        % loading in of underlay data
                        
                        handles.w = warndlg('Assuming your surface file has a hemisphere reference (i.e., lh, left, l., rh, right, rh.)');
                        id = strfind(lower(name1),'lr'); % some files will use the LR string, which we need to remove to check for l.
                        name1(id:id+1) = [];
                        
                        ss = {'left', 'lh', 'l.'};
                        ss2 = {'right', 'rh','r.'};
                        fun = @(s)~cellfun('isempty',strfind(lower({name1}),s));
                        out = cellfun(fun,ss,'UniformOutput',false);
                        out2 = cellfun(fun,ss2,'UniformOutput',false);
                        
                        % assume straightforward mapping of curves onto
                        % hemi (first set in cifti is LH and first surf
                        % file is LH)
                        curv1 = curv1Tmp;
                        curv2 = curv2Tmp;
                        
                        % now change the curves if necessary
                        if out{1} == 1 || out{2} == 1 || out{3} == 1 % this means surf1 is LH
                            if ~isempty(strfind(hdr.diminfo{1}.models{1}.struct,'RIGHT'))
                                curv1 = curv2Tmp;
                                curv2 = curv1Tmp;
                            end
                        elseif out2{1} == 1 || out2{2} == 1 || out2{3} == 1
                            if ~isempty(strfind(hdr.diminfo{1}.models{1}.struct,'LEFT'))
                                curv1 = curv2Tmp;
                                curv2 = curv1Tmp;
                            end
                        elseif (out{1} == 0 && out{2} == 0 && out{3} == 0) && (out2{1} == 0 && out2{2} == 0 && out2{3} == 0)
                            error('There is no reference to RH or LH in your surface file...please fix this')
                        end
                    catch
                        disp('It looks like your .nii file is probably not a CIFTI file (curvature)')
                    end
            end
        else
            switch ext3
                case '.gii'
                    hdr = gifti(curvFile{1});
                    curv1 = hdr.cdata;
                case '.curv'
        ...
        if isempty(cH)
            [handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1, curv1, vert2, face2, curv2);
        elseif cH == 1
            % we assume a patch error is because you've loaded in a CIFTI
            % file, which has different indexing. In this case, we also
            % need to adjust right hemisphere so it's not facing the same
            % way as the left!
                       
            % bring right hemisphere closer if necessary
            m2 = min(vert2(:,1));
            m1 = min(vert1(:,1));
            movF = (m1+1.5) - m2;
            vert2Fix = [vert2(:,1)+movF vert2(:,2) vert2(:,3)];
            
            [handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1-1, curv1, vert2Fix, face2-1, curv2,'true');
            
            [handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1-1, curv1, vert2Fix, face2-1, curv2,'true');
            handles.underlay.left.FaceColor = [0.5 0.5 0.5];
            handles.underlay.right.FaceColor = [0.5 0.5 0.5];
            %[handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1-1, curv1, vert2Fix, face2-1, curv2,'true');
            
            
            %[handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1-1, -1*extracted_left, vert2Fix, face2-1, -1*extracted_right,'true');
            cH = [];
        end
    else % if you selected one file just load that in and figure out which is negative
        [path1,name1,ext1] = fileparts(brainFile{1});
        [path2,name2,ext2] = fileparts(curvFile{1});
        switch ext1
            case '.inflated'
                [vert1,face1] = read_surf(brainFile{1});
            case '.gii'
        ...
        switch ext2
            ...
            case '.nii'
                try
                    ...
                    curv1Tmp(hdr.diminfo{1}.models{1}.vertlist + 1) = hdr.cdata(hdr.diminfo{1}.models{1}.start:hdr.diminfo{1}.models{1}.start+hdr.diminfo{1}.models{1}.count-1);
                    curv2Tmp = zeros([size(vert2,1),1]);
                    curv2Tmp(c.diminfo{1}.models{2}.vertlist + 1) = hdr.cdata(hdr.diminfo{1}.models{2}.start:hdr.diminfo{1}.models{2}.start+hdr.diminfo{1}.models{2}.count-1);
                    
                    handles.w = warndlg('Assuming CIFTI file is from HCP, in which case we need to invert the sulcal/gyral info');
                    %curv1Tmp = -1*curv1Tmp;
                    %curv2Tmp = -1*curv2Tmp;
                catch
                    disp('It looks like your .nii file is not a CIFTI file (curvature)')
                end
                
                % if this is the case, you need to figure out whether to
                % keep curv1 or curv2; start by identifying hemisphere of
                % surf file...
                id = strfind(lower(name1),'lr'); % some files will use the LR string, which we need to remove to check for l.
                name1(id:id+1) = [];
        ...
        if isempty(cH)
            [handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1, curv1);
        elseif cH == 1
            [handles.underlay, handles.brainFig] = plotUnderlay(vert1, face1-1, curv1);
        end
    end
end
...
if isfield(handles, 'underlay') % only allow for button to work if there's an underlay
    ...
    overlayFiles = uipickfiles('REFilter','\.nii.gz$|\.nii$|\.gii$','Prompt','Select one or more overlays')';
    ...
    if isa(overlayFiles,'cell')
        ...
        % file if necessary.
        addFile = 0; % counter for additional files we'll need to insert due to cifti files
        for filei = 1:length(overlayFiles) 
            ...
            switch handles.brainMap.Current{storedFileNum}.ext
                case {'.nii';'.nii.gz';'.gz'}
                    try % first we try for nifti-1 file
                        hdr = load_nifti(overlayFiles{filei}); % load in data
                        
                        if length(size(hdr.vol)) > 2
                            errordlg('Your map is 3D. Maybe it is in volume space and you should import it instead of loading it.')
                            error('See error dialgoue');
                        end
                    ...
                    catch % if nifti-1 assumption failed, it must be a cifti file
                       disp('Assuming overlay is a cifti file') 
                       addFile = addFile + 1;
                       
                       hdr = cifti_read(overlayFiles{filei});
                       d1 = zeros([hdr.diminfo{1}.models{1}.numvert,1]);
                       d1(hdr.diminfo{1}.models{1}.vertlist + 1) = hdr.cdata(c.diminfo{1}.models{1}.start:hdr.diminfo{1}.models{1}.start+hdr.diminfo{1}.models{1}.count-1);
                       
                       handles.brainMap.Current{storedFileNum}.Data = d1;
                       handles.brainMap.Current{storedFileNum}.limitMin = min(d1);
                       handles.brainMap.Current{storedFileNum}.limitMax = max(d1);
                       id = strfind(handles.brainMap.Current{storedFileNum}.name,'LR'); % update name
                       if ~isempty(strfind(hdr.diminfo{1}.models{1}.struct,'LEFT'))
                           handles.brainMap.Current{storedFileNum}.name(id) = 'LH';
                       elseif ~isempty(strfind(hdr.diminfo{1}.models{1}.struct,'RIGHT'))
                           handles.brainMap.Current{storedFileNum}.name(id) = 'RH';
                       end

                       d2 = zeros([hdr.diminfo{1}.models{2}.numvert,1]);
                       d2(hdr.diminfo{1}.models{2}.vertlist + 1) = hdr.cdata(hdr.diminfo{1}.models{2}.start:hdr.diminfo{1}.models{2}.start+hdr.diminfo{1}.models{2}.count-1);
                       
                       newFileNum = numSavedOverlays + length(overlayFiles) + addFile;
                       handles.brainMap.Current{newFileNum} = handles.defaultOptions;
                       handles.brainMap.Current{newFileNum}.path = handles.brainMap.Current{storedFileNum}.path;
                       handles.brainMap.Current{newFileNum}.ext = handles.brainMap.Current{storedFileNum}.ext;
                       handles.brainMap.Current{newFileNum}.name = handles.brainMap.Current{storedFileNum}.name;
                       if ~isempty(strfind(hdr.diminfo{1}.models{2}.struct,'LEFT'))
                           handles.brainMap.Current{newFileNum}.name(id) = 'LH';
                       elseif ~isempty(strfind(hdr.diminfo{1}.models{2}.struct,'RIGHT'))
                           handles.brainMap.Current{newFileNum}.name(id) = 'RH';
                       end
  
                       handles.brainMap.Current{newFileNum}.Data = d2;
                       handles.brainMap.Current{newFileNum}.limitMin = min(d2);
                       handles.brainMap.Current{newFileNum}.limitMax = max(d2);
                    end
            
                case '.gii'
                    hdr = gifti(overlayFiles{filei});
                    
                    handles.brainMap.Current{storedFileNum}.Data = hdr.cdata;
                    handles.brainMap.Current{storedFileNum}.limitMin = min(hdr.cdata);
                    handles.brainMap.Current{storedFileNum}.limitMax = max(hdr.cdata);
            end
...
if isfield(handles,'brainMap')
    ...
    if length(handles.overlaySelection.Value) == 1
        if handles.overlaySelection.Value == 1 % if you selected no overlay load defaults
            ...
            handles.outlineROIsButton.Value = 0;
            ...
            if strcmp(handles.brainMap.Current{handles.overlaySelection.Value - 1}.outlineROIs,'false')
                handles.outlineROIsButton.Value = 0;
            elseif strcmp(handles.brainMap.Current{handles.overlaySelection.Value - 1}.outlineROIs,'true')
                handles.outlineROIsButton.Value = 1;
            end
...
if length(handles.overlaySelection.Value) == 1 % reloading only works if you don't select multiple items
    if handles.overlaySelection.Value ~= 1
        ...
        try
            ...
            handles.w = warndlg('No extension in brainmap, probably import error');
        end
...
for i = 1:length(handles.brainMap.Current)
    ...
    if length(figPost) > length(figPre)
        ...
        for j = 1:length(figPost)
            if figPost(j).Number == C
                % take photo of it...
                saveas(figPost(j),[oName '_transparencyModulatedColorbar.png']);
                deleteFig = j;
            end
        end
    end
...
if handles.overlaySelection.Value ~= 1 && length(handles.overlaySelection.Value) == 1
    ...
    if get(hObject,'Value') == 0
        ...
        handles.brainMap.Current{handles.overlaySelection.Value - 1}.outlineROIs = 'false';
        handles.outlineROIsButton.Value = 0;
    end
...
% --- Executes on button press in outlineROIsButton.
function outlineROIsButton_Callback(hObject, eventdata, handles)
% hObject    handle to outlineROIsButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if handles.overlaySelection.Value ~= 1 && length(handles.overlaySelection.Value) == 1
    ...
    if get(hObject,'Value') == 0
        ...
        handles.brainMap.Current{handles.overlaySelection.Value - 1}.outlineROIs = 'true';
        handles.brainMap.Current{handles.overlaySelection.Value - 1}.outline = 'false';
        handles.outlineButton.Value = 0;
    end
...
% --------------------------------------------------------------------
function intersectAtlas_Callback(hObject, eventdata, handles)
% hObject    handle to intersectAtlas (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --------------------------------------------------------------------
function atlas2surf_Callback(hObject, eventdata, handles)
% hObject    handle to atlas2surf (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function interAtlas_Callback(hObject, eventdata, handles)
% hObject    handle to interAtlas (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
