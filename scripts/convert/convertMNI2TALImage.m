function [] = convertMNI2TALImage(inFiles,thresh,binarize)
...
for i = 1:length(inFiles)
    ...
    if strfind(ext,'.gz') == 1
        ...
        if binarize == 0
            writeROI(data(idx),voxelROI_niftiSpaceS_MM2TAL_Voxel,[inFiles{i}(1:end-4) '_TAL_thresh' num2str(thresh) '.nii'],[],'false',[],[],[],[])
        else
            writeROI(ones(size(voxelROI_niftiSpaceS_MM2TAL_Voxel,1),1),voxelROI_niftiSpaceS_MM2TAL_Voxel,[inFiles{i}(1:end-4) '_TAL_thresh' num2str(thresh) '.nii'],[],'false',[],[],[],[])
        end
    end
end
