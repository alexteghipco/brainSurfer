function [outFiles] = convertMNI2FSWithMask(inFile, smoothSteps, reps)
...
inNifti = load_untouch_nii(inFile); % load inFile
emptVox = find(inNifti.img == 0); % get empty voxels
valsVox = find(inNifti.img ~= 0); % get nonempty voxels

% write out both empty and nonempty voxels as new nifti files
inNifti.img = zeros(91,109,91);
inNifti.img(emptVox) = 1;
save_untouch_nii(inNifti,[inFile(1:end-4) '_EMPTY_MASK.nii'])

inNifti.img = zeros(91,109,91);
inNifti.img(valsVox) = 1;
save_untouch_nii(inNifti,[inFile(1:end-4) '_VALS_MASK.nii'])

% convert file, and both masks (i.e., empty and non-empty voxel nifti
% files)
convertMNI2FS([inFile(1:end-4) '_EMPTY_MASK.nii'],[]);
convertMNI2FS([inFile(1:end-4) '_VALS_MASK.nii'],[]);
...
emptyMask = load_nifti([path '/lh.MNI_' file '_EMPTY_MASK_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);
valMask = load_nifti([ path '/lh.MNI_' file '_VALS_MASK_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);

% smooth the masks
if smoothSteps > 0
    [lhVertex, ~] = read_surf([templateDir '/surf/lh.inflated']); % get vertex spatial coords
...
save_nifti(emptyMask,[path '/lh.MNI_' file '_SURFACE_MASK_' num2str(smoothSteps) '_SMOOTHING_STEPS_WITH_' num2str(reps) '_REPS_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);

% load in the original file projected into surface space and mask out
% vertices from the surface mask
data = load_nifti([path '/lh.MNI_' file '_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);
...
data.vol = outMask;
save_nifti(data,[path '/lh.MNI_' file '_SURFACE_MASKED_' num2str(smoothSteps) '_SMOOTHING_STEPS_WITH_' num2str(reps) '_REPS_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);
outFiles{1} = [path '/lh.MNI_' file '_SURFACE_MASKED_' num2str(smoothSteps) '_SMOOTHING_STEPS_WITH_' num2str(reps) '_REPS_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz'];


% repeat all of the same steps for right hemisphere now
emptyMask = load_nifti([path '/rh.MNI_' file '_EMPTY_MASK_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);
valMask = load_nifti([ path '/rh.MNI_' file '_VALS_MASK_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);

% smooth the masks
if smoothSteps > 0
    [rhVertex, ~] = read_surf([templateDir '/surf/rh.inflated']); % get vertex spatial coords
    emptyMaskIdx = find(emptyMask.vol ~= 0); % identify vertices to smooth
    valMaskIdx = find(valMask.vol ~= 0); % identify vertices to smooth
...
emptyMask.vol = threshMask; % save out these vertices as a surface mask
save_nifti(emptyMask,[path '/rh.MNI_' file '_SURFACE_MASK_' num2str(smoothSteps) '_SMOOTHING_STEPS_WITH_' num2str(reps) '_REPS_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);

% load in the original file projected into surface space and mask out
% vertices from the surface mask
data = load_nifti([path '/rh.MNI_' file '_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);
for i = 1:length(data.vol)
...
data.vol = outMask;
save_nifti(data,[path '/rh.MNI_' file '_SURFACE_MASKED_' num2str(smoothSteps) '_SMOOTHING_STEPS_WITH_' num2str(reps) '_REPS_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz']);

outFiles{2} = [path '/rh.MNI_' file '_SURFACE_MASKED_' num2str(smoothSteps) '_SMOOTHING_STEPS_WITH_' num2str(reps) '_REPS_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz'];
