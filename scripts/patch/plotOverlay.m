    ...
    if exist('dataModSpace1','var')
        for i = 1:length(opacityColorbar)
            if i < (length(opacityColorbar)/2)+1
                if isnan(opacityColorbar(i))
                    tMarks(i,1) = 1;
                    isnanList(i,1) = 1;
                else
                    idx = find(dataModSpaceScaled1 >= opacityColorbar(i));
                    tMarks(i,1) = idx(1);
                    isnanList(i,1) = 0;
                end
            elseif i > (length(opacityColorbar)/2)+1
                if isnan(opacityColorbar(i))
                    tMarks(i,1) = 1;
                    isnanList(i,1) = 1;
                else
                    idx = find(dataModSpaceScaled2 >= opacityColorbar(i));
                    tMarks(i,1) = idx(1);
                    isnanList(i,1) = 0;
                end
