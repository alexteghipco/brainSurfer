...
if (isempty(options.transparencyThresholds) == 0 || isempty(options.transparencyPThresh) == 0) && isempty(options.transparencyLimits) == 0
    if isempty(options.transparencyPThresh) == 1
    ...
    elseif isempty(options.pMap) == 0 && isempty(options.transparencyPThresh) == 0
...
if strcmp(options.outline,'true') == 1 || strcmp(options.binarizeClusters,'true') == 1 || options.clusterThresh > 0
    ...
    switch options.outline
        case 'true'
            ...
            for clusteri = 1:length(dataClust)
                ...
                overlayData(dataClust{clusteri}) = oVals{clusteri}; % this means the border of your ROI will now be filled with mean intensity of its cluster
            end
    end
    allThreshVert = vertcat(dataClust{:});
end
...
if isempty(minThreshNeg) == 1
    minThreshNeg = 0;
end
if isempty(minThreshPos) == 1
    minThreshPos = 0;
end
...
if options.growROI ~= 0
    ...
    if strcmp(options.outline,'true') == 0
        growVert = getClusterBoundary(dataClust, faces);
        
        for clusteri = 1:length(growVert)
            oVals{clusteri} = mean(overlayData(growVert{clusteri}));
            ...
            overlayData(growVert{clusteri}) = oVals{clusteri}; % this means the border of your ROI will now be filled with mean intensity of its cluster
        end
    ...
    while growSteps > 0
        for clusteri = 1:length(growVert)
            if options.growROI > 0
                ...
                overlayData(growVert{clusteri}) = oVal;
...
switch options.colorSampling
    ...
    case 'center on threshold'
        ...
        if (isempty(posTest) == 1 && isempty(negTest) == 0) || (isempty(posTest) == 0 && isempty(negTest) == 1)
            if isempty(posTest) == 1 && isempty(negTest) == 0
                cData = linspace(minThreshPos,options.limits(2),options.colorBins);
            end
            if isempty(posTest) == 0 && isempty(negTest) == 1
...
if isempty(options.figHandle) == 1 && strcmp(options.plotSwitch,'on')
...
if isempty(options.overlay) == 0 && strcmp(options.multiOverlay,'off') && strcmp(options.plotSwitch,'on')
    % but before that, save info about lighting and specular strength
    if isvalid(options.overlay)
        overProps.specStren = options.overlay.SpecularStrength;
        overProps.spexExp = options.overlay.SpecularExponent;
        overProps.specRef = options.overlay.SpecularColorReflectance;
        overProps.diff = options.overlay.DiffuseStrength;
        overProps.amb = options.overlay.AmbientStrength;
...
% data.
if options.binarize ~= 0 || strcmp(options.binarizeClusters, 'true') == 1 || strcmp(options.outline,'true') == 1 || strcmp(options.colorSampling,'center on threshold') == 1 || options.binarize ~= 0 || (options.smoothArea > 0 && options.smoothSteps > 0) || options.growROI ~= 0
...
switch options.inclZero
    case 'false'
        ...
        %         ocData(idx,1) = cMap(minv,1);
        %         ocData(idx,2) = cMap(minv,2);
        %         ocData(idx,3) = cMap(minv,3);
        %
        ocData(idx,1) = 0;
        ocData(idx,2) = 0;
        ocData(idx,3) = 0;
...
switch options.plotSwitch
    case 'on'
        ...
        if strcmp(options.multiOverlay,'on')
            overlay = patch('Faces',faces,'Vertices',underlay.(options.hemisphere).Vertices,'FaceVertexCData',ocData,'FaceAlpha',options.opacity,'AlphaDataMapping' ,'none','CDataMapping','direct','facecolor','interp','edgecolor','none');
        else
...
%% delete!!!
% options.colorbar = cbar;
% options.colorbar.TickLength = [0 0];
% imh = options.colorbar.Children(1);
% imh.AlphaData = opacityColorbar;
% imh.AlphaDataMapping = 'direct';
% options.colorbar.YTick = ticks;
% options.colorbar.YTickLabel = tickLabels;
